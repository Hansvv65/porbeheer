...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!$errors) {
    try {
      ...
      $pdo->prepare("UPDATE users SET verify_token_hash=?, verify_expires_at=? WHERE id=?")
          ->execute([$tokenHash, $exp, $uid]);

      $pdo->commit();

      // Link
      $baseUrl = rtrim((string)(getenv('APP_BASE_URL') ?: 'https://administratie.porzbeheer.nl'), '/');
      $link = $baseUrl . "/verify.php?token=" . urlencode($token) . "&email=" . urlencode($email);

      // Optionele bijlage (voorbeeld): public/assets/docs/voorwaarden.pdf
      $attachments = [];
      $voorwaarden = __DIR__ . '/assets/docs/voorwaarden.pdf';
      if (is_file($voorwaarden)) {
        $attachments[] = ['path' => $voorwaarden, 'name' => 'voorwaarden.pdf', 'type' => 'application/pdf'];
      }

      $html = "
        <p>Hoi " . h($username) . ",</p>
        <p>Je aanmelding voor Porbeheer is ontvangen.</p>
        <p>Bevestig eerst je e-mailadres via deze link (48 uur geldig):</p>
      ...
      auditLog($pdo, 'REGISTER_OK', 'auth/register', ['username' => $username, 'email' => $email, 'user_id' => $uid]);
      $success = 'Aanmelding ontvangen. Controleer je e-mail om je e-mailadres te bevestigen.';
    } catch (Throwable $e) {
      if ($pdo->inTransaction()) $pdo->rollBack();
      auditLog($pdo, 'REGISTER_FAIL', 'auth/register', ['username' => $username, 'email' => $email]);
      $errors[] = 'Aanmaken mislukt. Bestaat gebruikersnaam of e-mail al?';
    }
  }
}
