...
/*
 * budget_edit.php
 * Bewerk een begrotingspost (admin)
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } elseif (!in_array($row['kind'], ['income', 'expense', 'both'], true)) {
        $err = 'Ongeldig type.';
    } elseif ($amt < 0) {
        $err = 'Jaarbedrag mag niet negatief zijn.';
    }
...
if ($msg) {
    echo "<script>setTimeout(function(){ window.location.href = '/admin/budget.php'; }, 2000);</script>";
}
?>
    ...
    <div class="backdrop">
        <div class="wrap">
            <div class="topbar">
                <div class="brand">
                    <h1>Porbeheer</h1>
                    <div class="sub">POP Oefenruimte Zevenaar • begrotingspost</div>
                </div>
                <div class="userbox">
                    <div class="line1">Hallo <?= h($user['username'] ?? '') ?> · rol <?= h($role) ?></div>
                    <div class="line2">
                        <a href="/admin/budget.php">Begroting</a>
                ...
                <form method="post">
                    ...
                    <div class="grid2">
                        ...
                        <div class="row">
                            <label>Type</label>
                            <select name="kind">
                                <option value="income" <?= ($row['kind'] === 'income' ? 'selected' : '') ?>>Inkomsten</option>
                                <option value="expense" <?= ($row['kind'] === 'expense' ? 'selected' : '') ?>>Uitgaven</option>
                                <option value="both" <?= ($row['kind'] === 'both' ? 'selected' : '') ?>>Beide</option>
                            </select>
                        </div>
                        <div class="row">
